...
class App():
    ...
    def start(self):
        ...
        Label(self.master, text=label02).grid(row=5, column=0, sticky=W)
        Label(self.master, text=label03).grid(row=8, column=0, sticky=W)
        Label(self.master, text=label04).grid(row=9, column=0, sticky=W)
        ...
        self.filelocation2.grid(row=10,column=0)
        ...
        self.save_file.grid(row=10, column=1) #put it beside the filelocation textbox
        ...
        for text, item in RADIO_BUTTON1:
            ...
            self.radio1.grid(row=8, column=i+1)
            i += 1
    ...
    def load_dem(self):
        ...
        Label(self.master, text='DEM loaded.'
                                '                                          ').grid(row=2, column=0, sticky=E)

    def steady_state(self):
        self.GFM.reach_steady_state()

    def simulate(self):
        self.GFM.simulate(temp_change=self.w.get())
